...
% scale_factor = 10 / std(z(:));  % normalise spread if required
% z = z * scale_factor;
...
if n1 ~= n2
    ...
    nugget = n1/(10^12 - 1); % add nugget to make sure the matrix is well conditioned (adjust size depending on noise in data)
    d = sqrt(d2); % Euclidean distance
    R = (1 + sqrt(5) * d + (5/3) * d.^2) .* exp(-sqrt(5) * d); % Matern 5/2
    R = R + nugget*eye(n1);
    % R = exp(-d2) + nugget*eye(n1); % squared exponential
end
end
...
if nargin == 6 
    ...
    elseif d_lv==2
        ...
        lb = -10*ones(1, n_quan); ub = 10*ones(1, n_quan);
        for i = 1:length(dim_qual)
